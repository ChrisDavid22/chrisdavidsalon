...
const PROD_URL = 'https://www.chrisdavidsalon.com';
...
test('All PRODUCTION admin pages load with content', async ({ page }) => {
    for (const p of pages) {
        console.log(`Testing PRODUCTION: ${p.name}`);
        await page.goto(`${PROD_URL}${p.url}`);
        ...
        await page.screenshot({ path: `tests/screenshots/prod-${filename}.png`, fullPage: true });
    }
});

test('PRODUCTION Dashboard loads SEO scores and competitor comparison', async ({ page }) => {
    await page.goto(`${PROD_URL}/admin/index.html`);
    ...
    await page.screenshot({ path: 'tests/screenshots/prod-dashboard-complete.png', fullPage: true });
});

test('PRODUCTION Traffic page shows live GA4 data', async ({ page }) => {
    await page.goto(`${PROD_URL}/admin/traffic.html`);

    // Wait for GA4 data to load
    await page.waitForFunction(() => {
        const activeUsers = document.getElementById('activeUsers');
        return activeUsers && activeUsers.textContent !== '--';
    }, { timeout: 30000 });

    const activeUsers = await page.locator('#activeUsers').textContent();
    console.log('Active Users (30d):', activeUsers);
    expect(activeUsers).not.toBe('--');

    await page.screenshot({ path: 'tests/screenshots/prod-traffic-with-data.png', fullPage: true });
});
